...
const remove_interval = (element) => {
    let parent = $(element).closest('div.d-flex');
    if($('#manual_date').val() == "")
        delete TASK_DATA[edit_task_id][get_today_date()]['intervals'][$(parent).attr('id')];
    else
        delete TASK_DATA[edit_task_id][get_today_date($('#manual_date').val())]['intervals'][$(parent).attr('id')];
    $(parent).remove();
}
...
const prepare_task = () => {   
    ...
    const fill_todays_date = () => {
        ...
        $.each(ALL_HABITS, function(ind, ele){
            if(new Date(ALL_HABITS[ind]['habit_start_date']) > currentDate || new Date(ALL_HABITS[ind]['habit_end_date']) < currentDate || TASK_DATA[ind][fullDate] ||ALL_HABITS[ind]['repeating days'].includes(currentDate.getDay().toString()) == false){
                return;
            }
    ...
    draw_task_dt = () => {
        ...
        $.each(ALL_HABITS, function(ind, ele){
            ...
            let tot_dur = task_data['total_duration'];
            let arr = [];
            ...
            if(tot_dur == "--:--")
                ...
                arr.push(`${tot_dur.substring(0, tot_dur.indexOf(':'))}H ${tot_dur.substring(tot_dur.indexOf(':')+1)}M`)
            arr.push(TASK_DATA[ind][fullDate]['note'])
    ...
    const interval_proto = () => {
        return`
            <div class="d-flex justify-content-between align-items-end mb-4" id="IC-${interval_counter++}">
                <div class="row w-100 px-0 mx-0">
                    <div class="col">
                        <input type="time" class="form-control StartTime" name="Start Time" aria-describedby="helpId"/>
                    </div>
                    <div class="col">
                        <input type="time" class="form-control EndTime" name="End Time" aria-describedby="helpId"/>
                    </div>     
                </div>
                <div>
                    <button class="btn btn-danger" onclick="remove_interval(this)">Delete</button>
                </div>    
            </div>
        `
    }
    ...
    $('#add-interval').click(function () { 
        let intervals = $('#all-intervals').find('.row');
        if(Object.keys(intervals).length > 6)
            return;
        $('#all-intervals').append(interval_proto);
    })
    ...
    $(edit_task_form).submit(function (event) {
        ...
        let all_inp = $(edit_task_form).find('div.row');
        let note = $(edit_task_form).find('#Note').val();

        let total_duration = 0;
        $.each(all_inp, function (ind, ele) { 
            let interval_id = $(ele).parent().attr('id');
            let start_time = $(ele).find('.StartTime').val()
            let end_time = $(ele).find('.EndTime').val()
            let task_date_interval = TASK_DATA[edit_task_id][fullDate]['intervals']
            ...
            TASK_DATA[edit_task_id][fullDate]['intervals'][interval_id] = {
        ...
        let intervals = TASK_DATA[edit_task_id][fullDate]['intervals']
        $.each(intervals, function (ind, ele) { 
            let time = intervals[ind]['actual_duration'];
            let hr = parseFloat(time.substring(0,time.indexOf('.')));
            let min = parseFloat(time.substring(time.indexOf('.')+1));
            total_duration += (hr * 60) + min;
        })
        let hours = Math.floor(total_duration / 60);
        ...
        arr.push(`${hours}H ${minutes}M`) //total duration
        arr.push(note)
        arr.push('<div class="d-flex"><button data-bs-toggle="modal" data-bs-target="#task_edit_modal" class="btn btn-primary edit-task me-2">Edit</button></div>');         
        ...
        TASK_DATA[edit_task_id][fullDate]['note'] = note;
    ...
    $('tbody').click(function (event) {
        ...
        let cur_task_data = TASK_DATA[parent_row_id];
        

        if($(target).hasClass('edit-task')){
            edit_task_id = parent_row_id;
            let task_title = $('#taskTitle')
            let note = $(edit_task_form).find('#Note')
            task_title.val(ALL_HABITS[parent_row_id]['habit_title'])
            
            const fullDate = user_date
            let todays_detail = cur_task_data[fullDate]['intervals']; 
            let num_of_intervals = Object.keys(todays_detail).length;

            
            if(num_of_intervals != 0){
                while(num_of_intervals-- > 1){
                    $('#all-intervals').append(interval_proto);
                }
                let all_inp = $(edit_task_form).find('div.row');
                let ele_len = all_inp.length;
                let counter = 0;
                $.each(todays_detail, function(ind, ele){
                    $(all_inp[counter]).parent().attr('id', ind);
                    counter++;
                })
                $.each(all_inp, function(ind, ele){
                    let cur_row = $(ele).parent().attr('id')
                    $(ele).find('.StartTime').val(todays_detail[cur_row]["start_time"])
                    $(ele).find('.EndTime').val(todays_detail[cur_row]["end_time"])
                })
                interval_counter = Object.keys(todays_detail).length + 1;
            }
            note.val(cur_task_data[fullDate]['note']);
        ...
        if($(target).hasClass('task_comp_checkbox')){            
            ...
            if($(parent_row).hasClass('bg-success-subtle')){
                ...
                TASK_DATA[parent_row_id][user_date] = {
                    'intervals' :{},
                    'note':''
                }
                localStorage.setItem('data', JSON.stringify(ALL_HABITS))
...
const prepare_goal = () => {
    ...
    $(new_habit_form).submit(function(event){
        ...
        if(to_edit != -1)
            ...
            TASK_DATA[`H-${total_habit_counter}`] = {}
    ...
    const data = [];
      ...
      $.each(records, function(ind, ele){
        if(records[ind]['completed'] == 0)
            return;
        console.log(records[ind]['total_duration']);
        let duration = records[ind]['total_duration'].replace(':','.');
